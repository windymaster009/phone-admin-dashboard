...
import { AlertTriangle, ArrowUpRight, BadgeCheck, Clock3, HandCoins, Printer, Trash2, X } from 'lucide-react'
...
export default function PawnDetailModal({ pawn, onClose, onOpenAll, onAction, onDelete, canDelete = false }: { pawn: Pawn; onClose: () => void; onOpenAll?: () => void; onAction?: (action: PawnAction, payload: Record<string, unknown>) => Promise<void>; onDelete?: () => Promise<void>; canDelete?: boolean }) {
    ...
    <div className="modal-backdrop" role="presentation">
      <section className="detail-modal pawn-detail-modal surface-card" role="dialog" aria-modal="true" aria-labelledby="pawn-detail-title">
        ...
        {action && <form className={`pawn-action-form ${action === 'renew' ? 'renew-action-form' : ''}`} onSubmit={submitAction}>
          ...
          {action !== 'forfeit' && <label className="pawn-action-note"><span>Note <small className="optional-marker">Optional</small></span><textarea rows={2} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add a reference or payment note" /></label>}
