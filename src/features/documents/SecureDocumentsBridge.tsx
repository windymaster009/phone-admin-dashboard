      ...
      <div className="secure-document-content">
        <article className="surface-card secure-upload-card">
          ...
          <form onSubmit={uploadDocument}>
            ...
            <label className="secure-file-field"><span>File</span><input ref={fileInputRef} className="secure-file-input" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event) => setFile(event.target.files?.[0] || null)} required /><span className="secure-file-picker"><span className="secure-file-action"><Upload size={15} /> Choose file</span><strong title={file?.name}>{file ? file.name : 'No file selected'}</strong></span></label>
