...
const emptyCurrencySummary = (): CurrencySummary => ({
  lent: 0,
  expected: 0,
  paid: 0,
  outstanding: 0,
...
function ModalShell({ title, eyebrow, description, onClose, children }: {
  title: string
  eyebrow: string
  description: string
  onClose: () => void
...
function CreateLoanModal({ busy, error, onClose, onSubmit }: {
  ...
  const expected = principal + interestAmount

  return <ModalShell title="Create loan" eyebrow="Money lending" description="Record who borrowed money, the agreed amount, and when it must be repaid." onClose={onClose}>
    ...
    <form className="operation-form loan-create-form" onSubmit={onSubmit}>
      <div className="operation-form-grid">
        ...
        <label>Loan date<input name="loanDate" type="date" required defaultValue={dateInput(loanDate)} /></label>
        <label>Due date<input name="dueDate" type="date" required defaultValue={dateInput(defaultDueDate)} /></label>
        ...
        <label>Reason<input name="reason" placeholder="Personal loan, emergency, business..." /></label>
      ...
      <div className="loan-preview">
        ...
        <div><span>Total expected</span><strong>{money(expected, currency)}</strong></div>
      </div>
...
function LoanDetailModal({ detail, user, busy, error, onClose, onPayment, onDueDate, onCancel }: {
  ...
  return <ModalShell title={`${loan.loanNo} · ${loan.borrower.name}`} eyebrow="Loan record" description={`${dueDescription(loan)} · ${money(loan.remainingBalance, loan.currency)} remaining`} onClose={onClose}>
    {error && <div className="operation-modal-error"><AlertTriangle size={17} /> {error}</div>}
...
function SummaryAmount({ value }: { value: CurrencySummary[keyof CurrencySummary] }) {
  return <><strong>{money(Number(value || 0), 'USD')}</strong><small>{money(Number(value || 0), 'KHR')}</small></>
}

function LoanPage({ summary, setSummary }: { summary: LoanSummary; setSummary: (summary: LoanSummary) => void }) {
  ...
  }, [search, setSummary, status])

  useEffect(() => { void api<{ user: SessionUser }>('/auth/me').then((result) => setUser(result.user)).catch(() => undefined) }, [])
  useEffect(() => { const timeout = window.setTimeout(() => void loadLoans(), 180); return () => window.clearTimeout(timeout) }, [loadLoans])
  ...
  async function openDetail(loan: Loan) {
    setModalError('')
    try {
      setDetail(await api<LoanDetail>(`/loans/${loan._id}`))
    } catch (reason) {
  ...
  async function recordPayment(event: FormEvent<HTMLFormElement>) {
    ...
    setModalError('')
    try {
      const next = await api<LoanDetail>(`/loans/${detail.loan._id}/payments`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  return <div className="loan-workspace-bridge">
    ...
    <section className="loan-stats-grid">
      <article className="surface-card loan-stat"><span className="loan-stat-icon violet"><Banknote /></span><p>Total lent<SummaryAmount value={summary.byCurrency.USD.lent} /><em>{summary.counts.total} loan{summary.counts.total === 1 ? '' : 's'}</em></p></article>
      <article className="surface-card loan-stat"><span className="loan-stat-icon blue"><CircleDollarSign /></span><p>Outstanding<SummaryAmount value={summary.byCurrency.USD.outstanding} /><em>{summary.counts.open} still open</em></p></article>
      <article className="surface-card loan-stat"><span className="loan-stat-icon orange"><Clock /></span><p>Due soon<SummaryAmount value={summary.byCurrency.USD.dueSoon} /><em>{summary.counts.dueSoon} reminder{summary.counts.dueSoon === 1 ? '' : 's'}</em></p></article>
      <article className="surface-card loan-stat"><span className="loan-stat-icon rose"><AlertTriangle /></span><p>Overdue<SummaryAmount value={summary.byCurrency.USD.overdue} /><em>{summary.counts.overdue} need attention</em></p></article>
    </section>
    ...
    <article className="surface-card table-card page-table loan-table-card">
      ...
      <div className="table-scroll">
        <table><thead><tr><th>Loan</th><th>Borrower</th><th>Lent</th><th>Remaining</th><th>Due date</th><th>Status</th><th /></tr></thead><tbody>
          {visibleLoans.map((loan) => <tr key={loan._id} className={loan.status === 'OVERDUE' ? 'loan-overdue-row' : ''}>
...
export default function LoanWorkspaceBridge() {
  ...
  const loadSummary = useCallback(async () => {
    try {
      const result = await api<{ summary: LoanSummary }>('/loans/summary')
      setSummary(result.summary)
    } catch {
      // The bridge also exists on the sign-in screen; wait for an authenticated workspace.
    }
  }, [])

  useEffect(() => {
    let ownedNavHost: HTMLElement | null = null
    const locateTargets = () => {
      const main = document.querySelector<HTMLElement>('.main-content')
      if (main && main !== mainTarget) setMainTarget(main)
      ...
      if (!host) {
        ...
        const operations = groups.find((group) => group.querySelector('.nav-group-label')?.textContent?.trim() === 'Operations')
        if (operations) {
          host = document.createElement('span')
          host.className = 'loan-nav-portal-host'
          const buttons = Array.from(operations.querySelectorAll<HTMLElement>(':scope > button'))
          const pawnButton = buttons.find((button) => button.textContent?.includes('Pawn Management'))
          if (pawnButton) pawnButton.after(host)
          else operations.append(host)
          ownedNavHost = host
        }
      }
      if (host && host !== navTarget) setNavTarget(host)
    }
    ...
    const handleSidebarClick = (event: MouseEvent) => {
      ...
      if (target && !target.closest('.loan-nav-portal-host')) setActive(false)
    }
    const handlePopState = () => setActive(window.location.pathname === '/loans')
    ...
    return () => {
      ...
      ownedNavHost?.remove()
      mainTarget?.classList.remove('loan-route-active')
    }
  }, [mainTarget, navTarget])
  ...
  return <>
    ...
    {active && mainTarget && createPortal(<LoanPage summary={summary} setSummary={setSummary} />, mainTarget)}
  </>
}
